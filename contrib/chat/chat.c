...
void usage __P((void));
void log_mesg __P((const char *fmt, ...));
 ...
 */
void log_mesg __V((const char *fmt, ...))
{
    va_list args;
 ...
 *      set sending to 1 when sending (putting) this string.
 */
...
void chat_send (char *s)
{
	...
	if (verbose)
	    log_mesg("timeout set to %d seconds", timeout);
...
int get_char(void)
{
    ...
    default:
	log_mesg("warning: read() on stdin returned %d", status);
...
int put_char(int c)
{
    ...
    default:
	log_mesg("warning: write() on stdout returned %d", status);
...
int write_char (int c)
{
	...
	if (verbose) {
		...
		log_mesg(" -- write timed out");
	    else
		log_mesg(" -- write failed: %m");
	}
	return (0);
    }
    return (1);
...
int put_string (char *s)
{
    ...
    if (verbose) {
	if (quiet)
	    log_mesg("send (?????\?)");	/* backslash to avoid trigraph ??) */
	else
	    log_mesg("send (%v)", s);
    }
...
int get_string(char *string)
{
    ...
    if (verbose)
	log_mesg("expect (%v)", string);

    if (len > STR_LEN) {
	log_mesg("expect string is too long");
	exit_code = 1;
	return 0;
    }
	...
	*s++ = c;
	...
	if (alarmed && verbose)
	    log_mesg("warning: alarm synchronization problem");
    }
    ...
    if (verbose && printed) {
	if (alarmed)
	    log_mesg(" -- read timed out");
	else
	    log_mesg(" -- read failed: %m");
    }
...
/*
 * array = The address of the array of string pointers
 * end = The index of the next free entry before CLR_
 */
